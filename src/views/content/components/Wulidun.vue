...
const search_car_transport = () => {
  ...
  if (car_transport_select_value.value == "") {
    ...
    if (car_transport_select_way.value == "month") {
      start = moment(car_transport_select_value.value[0]).startOf("month").format("YYYY-MM-DD")
      if (
...
const junk_exportExcel = () => {
  ...
  getJunkForm(junk_export_start, junk_export_end, "五里墩");
};
//===============================================================================================================
//===============================================================================================================
const value = ref("");
...
console.log(Number((yAxis_alert.value[0] * 0.2)))
  ...
  getQuery("五里墩", "transporter", today, tomorrow, 1, 10).then(function (
    resp
  ) {
    data.value = resp;
    // patrols.value.splice(0, patrols.value.length);
