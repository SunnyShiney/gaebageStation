...
const search_car_transport = () => {
  ...
  if (
    ...
    querySitename.value = queryCarNum.value.substring(
      index + 1,
      queryCarNum.value.length
    );
    queryCarNum.value = queryCarNum.value.substring(0, index);

    carNumber = queryCarNum.value;
  }
  ...
  getTransportList(start, end, "big_stations", 1, 10000, carNumber);
  if (
    queryCarNum.value == "全部所有站点" ||
    queryCarNum.value == "全部" ||
    queryCarNum.value == ""
    ...
    queryCarNum.value = queryCarNum.value + " " + querySitename.value;
  }
};
    ...
    if (resp.status == 200) {
      ...
      if (carNumber == "big_stations") {
        ...
        for (var car in data) {
          if (
            carNumber == data[car].carNumber &&
            querySitename.value == data[car].siteName
          ) {
            var currentCar = {
              day: start + " 至 " + end,
              siteName: data[car].siteName,
...
getTransportList(transport_start, transport_start, "big_stations", 1, 10000, "big_stations");
    ...
    if (site_name_select_way.value == "month") {
      ...
      for (var date = 0; date <= 4; date++) {
        start = moment(site_name_select_value.value[0])
          .add(date, "months")
          .format("YYYY-MM-DD");
        if (moment(start).month() == moment().month()) {
          end = moment().format("YYYY-MM-DD");
        } else {
        ...
        //统计一个月的总量
        getSiteNameList(start, end, "big_stations", 1, 10000, date);
        //图标x轴标签展示月份
...
const junk_exportExcel = () => {
  ...
  getJunkForm(junk_export_start, junk_export_end, "big_stations");
};
...
const pull_page = (page) => {
  ...
  getQuery("金牛", "transporter", start, end, page, 10).then(function (resp) {
    data.value = resp;
  });
};
