...
const search_car_transport = () => {
  ...
  if (car_transport_select_value.value == "") {
    ...
    // car_transport_select_way.value = "";
      ...
      getTransportList(start, end, "西北桥", 1, 10000, carNumber);
...
getTransportList(transport_start, transport_start, "西北桥", 1, 10000, "all");
  ...
  if (site_name_select_value.value == "") {
    ...
    if (site_name_select_way.value == "month") {
      ...
      for (var date = 0; date <= 4; date++) {
        ...
        //统计一个月的总量
        getSiteNameList(start, end, "西北桥", 1, 10000, date);
        //图标x轴标签展示月份
...
const record_exportExcel = () => {
  ...
  getRecordForm(junk_export_start, junk_export_end, "西北桥");
};
...
const junk_exportExcel = () => {
  ...
  getJunkForm(junk_export_start, junk_export_end, "西北桥");
};
...
function changeDate() {
  start =
    ...
    "-" +
    value.value[1].getDate();
  getQuery("西北桥", "transporter", start, end, 1, 10000).then(function (resp) {
    total_records.value = resp.length;
    page_count = parseInt(resp.length) % 10;
 
  });
  getQuery("西北桥", "transporter", start, end, 1, 10).then(function (resp) {
    total_records.value = resp.length;
    page_count = parseInt(resp.length) % 10;
    data.value = resp;
  });
...
onBeforeMount(() => {
  ...
  getQuery("西北桥", "transporter", today, tomorrow, 1, 10000).then(function (
    ...
    for (let i = 1; i < 7; i++) {
      ...
      getQuery("西北桥", "transporter", cal_day, cal_time, 1, 10000).then(
        function (resp) {
          for (let i = 0; i < resp.length; i++) {
  ...
  // getQuery("西北桥", "transporter", today, tomorrow, 1, 10).then(function (resp) {
  //   data.value = resp;
  //   // patrols.value.splice(0, patrols.value.length);
  //   // total_records.value = parseInt(resp.total);
  //   // page_count = parseInt(resp.data.data.pages);
  ...
  getQuery("西北桥", "transporter", moment().format("YYYY-MM-DD"), moment().add(1,"d").format("YYYY-MM-DD"), 1, 10000).then(function (resp) {
    total_records.value = resp.length;
    page_count = parseInt(resp.length) % 10;
 
  });
  getQuery("西北桥", "transporter", moment().format("YYYY-MM-DD"), moment().add(1,"d").format("YYYY-MM-DD"), 1, 10).then(function (resp) {
    data.value = resp;
  });
...
const pull_page = (page) => {
  ...
  getQuery("西北桥", "transporter", start, end, page, 10).then(function (resp) {
    data.value = resp;
