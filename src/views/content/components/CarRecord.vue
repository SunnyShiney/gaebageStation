<template>
  <el-container>
    ...
    <el-main>
      ...
      <div class="mapDiv">
        <tdt-map :center="state.center" :zoom="state.zoom" mapStyle="black">
          <tdt-polygon
            :path="polygon.path"
            :opacity="0.8"
          ...
          ></tdt-polygon>
          <tdt-marker
            :position="current_car.location"
            :icon="current_car.icon"
          ></tdt-marker>
          <tdt-polyline
            v-model:path="current_car.current_path"
            color="#FF0000"
            :opacity="0.8"
            :visible="current_car.carVisible"
          ></tdt-polyline>
        </tdt-map>
      </div>
    </el-main>
...
const state = reactive({
  center: [104.05740358713781, 30.732356042874134],
  zoom: 12,
});
const month_before =
...
onMounted(() => {
  if (route.query.carNumber) {
    carValue.value = route.query.carNumber;
...
function showCars() {
  ...
  getCarGps(carValue.value, start, end).then((data) => {
    console.log(data);
    if (data == "error: 车牌号不存在或查询操作太频繁") {
      ElMessage({
        message: h("p", null, [h("span", null, "车辆在指定时间内暂无轨迹！")]),
        type: "error",
      });
    } else {
      current_car.location = [
        data[data.length - 1].longitude * 1,
        data[data.length - 1].latitude * 1,
      ];
      var pointList = [];
      for (let i = 0; i < data.length; i++) {
        var point = [data[i].longitude * 1, data[i].latitude * 1];

        pointList.push(point);
      }
      console.log(pointList);
      current_car.current_path = pointList;
      state.center = current_car.location;
...
function changeDate() {
  start =
    ...
    "T" +
    value.value[0].getHours().toString().padStart(2, 0) +
    ":" +
    value.value[0].getMinutes().toString().padStart(2, 0) +
    ":" +
    value.value[0].getSeconds().toString().padStart(2, 0);
    ...
    "T" +
    value.value[1].getHours().toString().padStart(2, 0) +
    ":" +
    value.value[1].getMinutes().toString().padStart(2, 0) +
    ":" +
    value.value[1].getSeconds().toString().padStart(2, 0);
