...
getTransportList(transport_start, transport_start, "红星", 1, 10000, "all");
  ...
  if (site_name_select_value.value == "") {
    ...
    if (site_name_select_way.value == "month") {
      ...
      for (var date = 0; date <= 4; date++) {
        ...
        //统计一个月的总量
        getSiteNameList(start, end, "红星", 1, 10000, date);
        //图标x轴标签展示月份
...
const record_exportExcel = () => {
  ...
  getRecordForm(junk_export_start, junk_export_end, "红星");
};
...
const junk_exportExcel = () => {
  ...
  getJunkForm(junk_export_start, junk_export_end, "红星");
};
...
onBeforeMount(() => {
  ...
  getQuery("红星", "transporter", moment().format("YYYY-MM-DD"), moment().add(1,"d").format("YYYY-MM-DD"), 1, 10000).then(function (resp) {
    total_records.value = resp.length;
    page_count = parseInt(resp.length) % 10;
 
  });
  getQuery("红星", "transporter", moment().format("YYYY-MM-DD"), moment().add(1,"d").format("YYYY-MM-DD"), 1, 10).then(function (resp) {
    data.value = resp;
  });
