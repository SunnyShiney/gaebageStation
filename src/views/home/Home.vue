<template>
  <body
    style="background-image: url(/images/bg.jpg);no-repeat top center;height:auto"
  >
    <Header
      style="
        height: 1.25rem;
    ...
    <div class="mainDiv">
      ...
      <div class="columns">
        ...
        <div class="mapDiv">
          ...
          >
            <!-- <tdt-control position="topleft" :visible="state.visible">
          <div class="chart">
          ...
          <div class="chart">
            ...
            <template v-for="car in carList">
              <tdt-marker
                ...
                :icon="car.icon"
              ></tdt-marker>
              <el-dialog v-model="car.carVisible" title="车辆今日轨迹">
                <el-descriptions
                  class="margin-top"
                  title="车辆信息"
                  :column="3"
                ...
                >
                  ...
                  <el-descriptions-item>
                    <template #label>
                      <div class="cell-item">司机姓名</div>
                    </template>
                    待完善
                  </el-descriptions-item>
                  <el-descriptions-item>
                    <template #label>
                      <div class="cell-item">所属公司</div>
                    </template>
                    {{ car.company }}
                  </el-descriptions-item>
                  <el-descriptions-item>
                    <template #label>
                      <div class="cell-item">最后记录gps时间</div>
                    </template>
                    {{ car.lastTime }}
                  </el-descriptions-item>
                </el-descriptions>
                <div class="mapDiv" style="width: 100%; height: 600px">
                  <tdt-map
                    :center="car.location"
                    :zoom="state.zoom"
                    :scrollWheelZoom="true"
            ...
            <el-dialog v-model="hongxingVisible" title="红星垃圾站今日数据">
              <Hongxing style="width: 100%" />
            </el-dialog>
            <el-dialog v-model="xihuaVisible" title="西华垃圾站今日数据">
              <Xihua style="width: 100%" />
            </el-dialog>
            <el-dialog v-model="honghuayanVisible" title="红花堰垃圾站今日数据">
            ...
            >
              <div class="card">
                <div class="card-body" style="text-align: center">
                  ...
                  <div
                    class="float-end"
                    style="margin-top: 1vh; display: inline-block; float: none"
                  >
                    <el-pagination
                      background
...
import Xihua from "@/views/content/components/Xihua.vue";
...
const hongxingVisible = ref(false);
const honghuayanVisible = ref(false);
const wulidunVisible = ref(false);
const wukuaishiVisible = ref(false);
const xibeiqiaoVisible = ref(false);
const jinquanVisible = ref(false);
const quanshuiVisible = ref(false);
const huangzhongVisible = ref(false);
...
const changeColorList = reactive([]);
...
const banshichuNoneSelectedStyle = ref(
  "text-align: center; padding-top: 10px; padding-bottom:10px; "
);
const banshichuSelectedStyle = ref(
  "text-align: center; padding-top: 10px; padding-bottom:10px; background-color: #0D6EFD; color: white;"
);
// 车辆的状态
      ...
      if (tianfu_car.has(resp[car].carNumber.toUpperCase())) {
        ...
        if (gps_time + 4 * 60 * 60 * 1000 > time) {
          ...
          var currentCar = {
            ...
            location: [resp[car].longitude * 1, resp[car].latitude * 1],
          };
    ...
    for (let i = 0; i < resp.length; i++) {
      ...
      if (resp[i].company == "仁和星牛") {
        ...
        if (!renhe_car.has(resp[i].carNumber.toUpperCase())) {
          renhe_car.set(resp[i].carNumber.toUpperCase(), 0);
        }
      }
      if (resp[i].company == "天府环境") {
        ...
        if (!tianfu_car.has(resp[i].carNumber.toUpperCase())) {
          tianfu_car.set(resp[i].carNumber.toUpperCase(), 0);
        }
      }
    }
  });
      ...
      for (var car in data) {
        ...
        if (car < 13) {
          var currentCar = {
            ...
            location: [data[car].longitude * 1, data[car].latitude * 1],
            company: "仁和星牛",
            icon: {
...
function showCar(car) {
  getCarGps(car.carNumber, today, current_time).then((data) => {
    ...
    for (let i = 0; i < data.length; i++) {
      let position = [data[i].longitude * 1, data[i].latitude * 1];
      car.path.push(position);
    }
...
const station_xibeiqiao = reactive({
  position: [104.067589, 30.691194],
  visible: true,
  icon: {
    iconUrl: require("@/assets/icon_marker_grey.jpg"),
    iconSize: [25, 35],
    iconAnchor: [9, 26],
  },
});
const station_jinquan = reactive({
  position: [104.006995, 30.709358],
  visible: true,
  icon: {
    iconUrl: require("@/assets/icon_marker_grey.png"),
    iconSize: [35, 35],
    iconAnchor: [9, 26],
  },
});

const station_quanshui = reactive({
  position: [104.068824, 30.736328],
  visible: true,
});
const station_xingsheng = reactive({
  position: [104.02199, 30.736284],
  visible: true,
});
const station_huangzhong = reactive({
  position: [104.018394, 30.695559],
  visible: true,
});
const station_yingmenkou = reactive({
  position: [104.044002, 30.707915],
  visible: true,
});
const station_xihua = reactive({
  position: [104.009131, 30.754915],
  visible: true,
  icon: {
    iconUrl: require("@/assets/icon_marker.jpg"),
    iconSize: [35, 45],
    iconAnchor: [9, 26],
  },
});
const station_hongxing = reactive({
  position: [104.093035, 30.75222],
  visible: true,
  icon: {
    iconUrl: require("@/assets/icon_marker.jpg"),
    iconSize: [35, 45],
...
const station_honghuayan = reactive({
  position: [104.085243, 30.690125],
  visible: true,
});
const station_wukuaishi = reactive({
  position: [104.072792, 30.709706],
  visible: true,
});
const station_wulidun = reactive({
  position: [104.04311, 30.694781],
  visible: true,
});
//地图在页面中的视野大小以及位置
const state = reactive({
  //中心点的经纬度，第一个是经度，第二个是维度
  center: [104.05740358713781, 30.72735604287413],
  //视野大小，数字越小视野越宽阔，数字越大视野越局部
  zoom: 12,
...
var site_name_sum = ref(0);
var site_name_sum1 = ref(0);
const getStationsTotal = (start, end, site_name, pageNum, pageSize, date) => {
  axios({
    url:
    ...
    //   if (resp.status == 200) {
    //     var data = resp.data.data.records;
    //     site_name_sum1.value = 0;
    //     for (let i = 0; i < data.length; i++) {
    //       site_name_sum1.value = data[i].totalWeight + site_name_sum1.value;
    //     }

    //     site_name_sum1.value =
    //       Math.floor((site_name_sum1.value / 1000) * 100) / 100;

    //     towOption.series[1].data[date].value = Number(site_name_sum.value.toFixed(0));
    //     console.log("多少啦"+ Number(site_name_sum.value.toFixed(0)))

    //     towOption.series[0].data[date].value = Number(site_name_sum1.value.toFixed(0));
    //     console.log("总总哦在那个" + towOption.series[1].data[date].value)
...
const create_tow_data = () => {
  ...
  //     total_tow.value = 0;
  //     for (let i = 0; i < resp.length; i++) {
  //       total_tow.value = resp[i].netWeight + total_tow.value;
  //     }
  //     total_tow.value = Math.floor((total_tow.value / 1000) * 100) / 100;
  //     //更改图表上对应位置的数据
  //     towOption.xAxis[0].data[0].value=moment().add(-4, "d").format("YYYY-MM-DD");
  //     towOption.series[0].data[0].value = Number(total_tow.value.toFixed(0));
  //     honghuayan = Number(total_tow.value.toFixed(0));
  //     // console.log(towOption.series[0].data[0].value+"第一个")
  //     //更新页面上的图表
  //     // tow_chart.setOption(towOption);
  //         getQuery(
  //     "西华",
  ...
  //     total_tow.value = Math.floor((total_tow.value / 1000) * 100) / 100;
  //     //更改图表上对应位置的数据
  //     towOption.series[0].data[0].value = Number(total_tow.value.toFixed(0))+towOption.series[0].data[0].value ;
  //     honghuayan = Number(total_tow.value.toFixed(0));
  //     // console.log(towOption.series[0].data[0].value+"第一个")
  //     //更新页面上的图表
  //     // tow_chart.setOption(towOption);
  ...
  //     total_tow.value = Math.floor((total_tow.value / 1000) * 100) / 100;
  //     //更改图表上对应位置的数据
  //     towOption.series[0].data[1].value = Number(total_tow.value.toFixed(0));
  //     wukuaishi = Number(total_tow.value.toFixed(0));
  //     //更新页面上的图表
  //     // tow_chart.setOption(towOption);
  //         getQuery(
  //     "西华",
  ...
  //     total_tow.value = Math.floor((total_tow.value / 1000) * 100) / 100;
  //     //更改图表上对应位置的数据
  //     towOption.series[0].data[1].value = Number(total_tow.value.toFixed(0))+towOption.series[0].data[1].value;
  //     wukuaishi = Number(total_tow.value.toFixed(0));
  //     //更新页面上的图表
  //     // tow_chart.setOption(towOption);
  //   });
  ...
  //     total_tow.value = Math.floor((total_tow.value / 1000) * 100) / 100;
  //     //更改图表上对应位置的数据
  //     towOption.series[0].data[2].value = Number(total_tow.value.toFixed(0));
  //     wulidun = Number(total_tow.value.toFixed(0));
  //     //更新页面上的图表
  //     // tow_chart.setOption(towOption);
  //         getQuery(
  //     "西华",
  ...
  //     total_tow.value = Math.floor((total_tow.value / 1000) * 100) / 100;
  //     //更改图表上对应位置的数据
  //     towOption.series[0].data[2].value = Number(total_tow.value.toFixed(0))+towOption.series[0].data[2].value ;
  //     wulidun = Number(total_tow.value.toFixed(0));
  //     //更新页面上的图表
  //     // tow_chart.setOption(towOption);
  //   });
  ...
  //     total_tow.value = Math.floor((total_tow.value / 1000) * 100) / 100;
  //     //更改图表上对应位置的数据
  //     towOption.series[0].data[3].value = Number(total_tow.value.toFixed(0));
  //     hongxing = Number(total_tow.value.toFixed(0));
  ...
  //       }
  //       total_tow.value = Math.floor((total_tow.value / 1000) * 100) / 100;
  //       xihua = Number(total_tow.value.toFixed(0));
  //       // console.log(xihua)
  //       //更改图表上对应位置的数据
  // towOption.series[0].data[3].value = Number(total_tow.value.toFixed(0))+towOption.series[0].data[3].value;
  ...
  //     total_tow.value = Math.floor((total_tow.value / 1000) * 100) / 100;
  //     //更改图表上对应位置的数据
  //     towOption.series[0].data[4].value = Number(total_tow.value.toFixed(0));
  //     hongxing = Number(total_tow.value.toFixed(0));
  ...
  //       // console.log(xihua)
  //       //更改图表上对应位置的数据
  // towOption.series[0].data[4].value = Number(total_tow.value.toFixed(0))+towOption.series[0].data[4].value;
  //       // console.log(wukuaishi+"墩")
  //       // tow_chart.setOption(towOption);
  //     });
  //   });
  ...
  //     total_tow.value = Math.floor((total_tow.value / 1000) * 100) / 100;
  //     //更改图表上对应位置的数据
  //     towOption.series[0].data[5].value = Number(total_tow.value.toFixed(0));
  //     hongxing = Number(total_tow.value.toFixed(0));
  ...
  //       // console.log(xihua)
  //       //更改图表上对应位置的数据
  // towOption.series[0].data[5].value = Number(total_tow.value.toFixed(0))+towOption.series[0].data[5].value;
  //       // console.log(wukuaishi+"墩")
  //       // tow_chart.setOption(towOption);
  //     });
  //   });
  ...
  //     total_tow.value = Math.floor((total_tow.value / 1000) * 100) / 100;
  //     //更改图表上对应位置的数据
  //     towOption.series[0].data[6].value = Number(total_tow.value.toFixed(0));
  //     hongxing = Number(total_tow.value.toFixed(0));
  ...
  //       // console.log(xihua)
  //       //更改图表上对应位置的数据
  // towOption.series[0].data[6].value = Number(total_tow.value.toFixed(0))+towOption.series[0].data[6].value;
  //       // console.log(wukuaishi+"墩")
  //       tow_chart.setOption(towOption);
  //     });
  //   });
...
const create_renhe_data = () => {
  ...
  getAllGps().then((resp) => {
    ...
    renheOption.series[0].data[0].value = num;
    renheOption.series[0].data[1].value = renhe_car.size - num;
    //更新页面上的图表
    renhe_chart.setOption(renheOption);
  });
};
...
const create_tianfu_data = () => {
  ...
  getAllGps().then((resp) => {
    ...
    tianfuOption.series[0].data[0].value = num;
    tianfuOption.series[0].data[1].value = tianfu_car.size - num;
    //更新页面上的图表
    tianfu_chart.setOption(tianfuOption);
  });
};
...
//Dom即将销毁，做一些清理操作
onBeforeUnmount(() => {
  window.removeEventListener("resize", small_chart.resize);
  if (small_chart) {
    small_chart.dispose();
    small_chart = null;
  }
  window.removeEventListener("resize", tow_chart.resize);
  if (tow_chart) {
    tow_chart.dispose();
    tow_chart = null;
  }
  ...
  window.removeEventListener("resize", renhe_chart.resize);
  if (renhe_chart) {
    renhe_chart.dispose();
    renhe_chart = null;
  }
...
::v-deep .panels .el-table thead tr th.is-leaf {
  border: 0px solid #ebeef5;
  border-right: none;
}

::v-deep .panels .el-table {
  border: 0;
  th,
  tr,
  td {
...
::v-deep .panels .el-table tr {
  background-color: transparent !important;
}

::v-deep .panels .el-table tbody tr:hover > td {
  background-color: rgba($color: #03487b, $alpha: 0.5) !important;
}

::v-deep .panels .el-table–enable-row-transition .el-table__body td,
.el-table .cell {
  background-color: transparent;
}
